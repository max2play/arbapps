...
"""
    ...
    Added Control of Squeezebox Server
    Stefan Rick 2017 Max2Play
...
class SqueezeSpectrumAnalyser(Application):
        ...
        for event in self.arbalet.touch.get():
            ...
            if self.is_touched == False:            
                ...
                if event['key']=='next':                    
                    ...
                    self.model.write("Play {}".format(self.squeezeplayer.get_track_title()), 'blue')
                    self.renderactive = True 
                elif event['key']=='volup':
                    self.squeezeplayer.volume_up(5)
                elif event['key']=='play':
                    self.squeezeplayer.toggle()
                elif event['key']=='voldown':
                    self.squeezeplayer.volume_down(5)
                    print("Volume Down")
                elif event['key']=='exit':
                    print("Exit")
                    self.exit = True
                    ...
                    print("Previous Song in Playlist")
    ...
    def run(self):
        ...
        while stream.is_active() and self.exit == False:
            self.process_events()                
